...
compSlopes <- function(mdl,method=stats::p.adjust.methods,
  ...
  if (lmtype$type!="IVR")
    FSA:::STOP("Function only works for dummy variable regressions.")
  if (lmtype$Enum>2)
  ...
  if (num.lvls<3) FSA:::WARN("Function not needed with fewer than three levels.")
  ...
  names(cdf)[3:4] <- names(sdf)[3:4] <- FSA:::iCILabel(conf.level)
...
compIntercepts <- function(mdl,common.cov=mean(x),
  ...
  ## Perform some checks
  lmtype <- FSA:::iTypeoflm(mdl)
  if (lmtype$type!="IVR") FSA:::STOP("Function only works for dummy variable regressions.")
  if (lmtype$Enum>2) FSA:::STOP("Function only works for dummy variable regressions\n with one factor and one covariate variable.")
  if (length(attr(stats::terms(mdl),"term.labels"))>2) FSA:::WARN("Removed an interaction term from 'mdl' (i.e., assumed\n parallel lines) to test intercepts.\n")
  ## isolate model information
  y <- lmtype$mf[,lmtype$Rpos]
  x <- lmtype$mf[,lmtype$ENumPos]
  g <- lmtype$mf[,lmtype$EFactPos]
  if (length(levels(g))<3) FSA:::WARN("Function not needed with fewer than three levels.")
  ...
  # Rename the confidence interval columns
  names(cdf)[3:4] <- FSA:::iCILabel(conf.level)
